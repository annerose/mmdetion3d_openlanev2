...
class OpenLaneV2SubsetADataset(Custom3DDataset):
    ...
    def get_ann_info(self, index):
        ...
        # fake annotations
        if frame.get_annotations_lane_centerlines() is None:
            return {'gt_lc': np.zeros((2, 2, 3)),
                    'gt_lc_labels': np.zeros(2),
                    'gt_te': np.zeros((2, 4)),
                    'gt_te_labels': np.zeros(2),
                    'gt_topology_lclc': np.zeros((2, 2)),
                    'gt_topology_lcte': np.zeros((2, 2))}
    ...
    def format_preds(self, results):
        ...
        predictions = {
            ...
            'institution / company': 'dummygroup',
        ...
        for index, result in enumerate(results):
            ...
            for i, (bbox, confidence) in enumerate(zip(*pred_te)):
                prediction['traffic_element'].append({
                    'id': i + 2000,
                    'attribute': bbox[-1].astype(np.float16),
                    'points': bbox[:-1].reshape(2, 2).astype(np.float16),
                    'confidence': confidence.astype(np.float16),
                })

            # topology

            prediction['topology_lclc'] = result['pred_topology_lclc'].astype(np.float16)
            prediction['topology_lcte'] = result['pred_topology_lcte'].astype(np.float16)
    ...
    def visualize(self, pred_dict, visualization_dir, visualization_num, confidence_threshold=0.3, **kwargs):
        ...
        for index, (key, prediction) in enumerate(pred_dict['results'].items()):
            ...
            pred_result = {
                ...
                'authors': pred_dict['authors'],
                'e-mail': pred_dict['e-mail'],
                'institution / company': pred_dict['institution / company'],
                'country / region': pred_dict['country / region'],
                'results': {
                    key: {
                        'predictions': prediction,
                    }
...
if __name__ == '__main__':
    ...
    dataset = OpenLaneV2SubsetADataset(data_root, meta_root, collection, None, test_mode)
    print(dataset)

    pf = open(r'work_dirs\baseline\raw_result.pkl', 'rb')
    raw_data = pickle.load(pf)

    new_result = dataset.format_preds(raw_data)

    file_out = open(r'E:\work\code\mmdetection3d-1.0.0rc6\work_dirs\baseline_large\result_e23_submitted_2.pkl', 'wb')
    pickle.dump(new_result, file_out,   protocol=pickle.DEFAULT_PROTOCOL)
    file_out.close()